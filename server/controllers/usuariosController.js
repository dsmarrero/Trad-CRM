...
async function actualizarConfig(req, res) {
  try {
    const { tarifa_traduccion, tarifa_jurada, tarifa_revision, tarifa_edicion_hora } = req.body;
    const resultado = await pool.query(
      `UPDATE usuarios SET
      ...
      [tarifa_traduccion, tarifa_jurada, tarifa_revision, tarifa_edicion_hora, req.usuario.id]
    );
    res.json(resultado.rows[0]);
  } catch (err) {
