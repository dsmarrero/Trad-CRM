...
async function crear(req, res) {
  try {
    ...
    const resultado = await pool.query(
      ...
      [cliente_id, idioma_origen, idioma_destino, tipo_documento, estado || 'recibido', fecha_entrega, precio, notas]
    );
...
async function actualizar(req, res) {
  try {
    ...
    const resultado = await pool.query(
      ...
      [cliente_id, idioma_origen, idioma_destino, tipo_documento, estado, fecha_entrega, precio, notas, id]
    );
    if (resultado.rows.length === 0) {
      return res.status(404).json({ error: 'Encargo no encontrado' });
    }
